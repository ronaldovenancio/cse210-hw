...
public class ReflectingActivity : Activity
{
    ...
    {
        Console.WriteLine();  //this command inserts blank line to start
    ...
    {
        _useQuestionsList.AddRange(_questionList); //this command creates a new list that can be destroyed each time.
        ...
        while (timer.Elapsed.TotalSeconds < seconds)
        {
            if (_useQuestionsList.Count != 0)
            {
                ...
                Console.Write($"\n>> {question}  ");
                _useQuestionsList.Remove(question);  //this command removes question from list so it can not be used again
            }
            spinner.ShowSpinner();
        }
        timer.Stop();
